import type { CatalogCardPageData } from '@/lib/catalogPages'
...
// Imagen Open Graph (1200x630) de la ficha de catálogo (/carta/…): la carta
// real a la izquierda y a la derecha nombre, set, precio desde y publicaciones.
// Se renderiza con @vercel/og (Satori): solo CSS inline.
export default function CatalogCardOgImage({
  data
}: {
  data: CatalogCardPageData | null
}) {
  if (!data) {
    return (
      <div
  ...
  return (
    ...
    >
      ...
      >
        ...
        {data.rarity && (
          <p style={{ fontSize: 22, color: '#94a3b8', margin: '8px 0 0' }}>{data.rarity}</p>
        )}
        ...
        >
          <p style={{ fontSize: 16, letterSpacing: 3, margin: 0, color: '#34d399' }}>
            {data.listingCount > 0 ? 'DESDE' : 'SIN PUBLICACIONES'}
          </p>
          <p style={{ fontSize: 38, fontWeight: 800, margin: '6px 0 0', color: '#ffffff' }}>
            {data.minPrice != null ? fmt(data.minPrice, data.currency) : 'Consultar precio'}
          </p>
        </div>

        <p style={{ fontSize: 20, color: '#64748b', margin: '32px 0 0' }}>
          {data.listingCount > 0
            ? `${data.listingCount} publicación${data.listingCount !== 1 ? 'es' : ''} en venta/cambio`
            : 'Precios, claims y coleccionistas en TCG Claim'}
        </p>
      </div>
    </div>
  )
